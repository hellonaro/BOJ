...
그들은 M과 N보다 작거나 같은 두 개의 자연수 x, y를 가지고 
...
int main() {
	int T, M, N, x, y;
	int big, small, big_year, small_year;
	int count, result, invalid;
	int gcd, lcm;
	...
	for (int i = 0; i < T; i++) {
		...
		for (int j = 0; j <= count; j++) {
			int temp = j * big + big_year - small_year;
			if (temp % small == 0) {
				invalid = 0;
				result = temp+small_year;
				break;
			}
		}
		result = invalid ? -1 : result;
		printf("%d\n", result);
	}
}